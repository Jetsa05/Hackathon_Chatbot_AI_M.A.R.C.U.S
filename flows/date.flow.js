...
const promptBase = `
    ...
    Si la disponibilidad es true, entonces responde algo como: La fecha solicitada la tienes libre. El evento sería el Jueves 30 de mayo 2024 a las 10:00 a.m.
    Si la disponibilidad es false, entonces recomenda la siguiente fecha disponible que te dejo al final del prompt, suponiendo que la siguiente fecha disponible es el Jueves 30, responde con este formato: La fecha y horario solicitados ya los tienes ocupados, te puedo ofrecer el Jueves 30 de mayo 2024 a las 11:00 a.m.
    ...
    .addAnswer("Confirmas la fecha propuesta? Responde unicamente con 'si' o 'no'", { capture: true },
        async (ctx, ctxFn) => {
            if (ctx.body.toLowerCase().includes("si")) {
                ...
                await ctxFn.endFlow("Evento cancelado. Vuelve a solicitar agendar para elegir otra fecha.")
            }
        })

const dateFlow = addKeyword(EVENTS.ACTION)
    .addAnswer("¿Cuándo sería la fecha del evento que quieres agendar?", { capture: true })
    .addAnswer("Revisando tu disponibilidad...", null,
        async (ctx, ctxFn) => {
            ...
            if (solicitedDate.includes("false")) {
                return ctxFn.endFlow("No se pudo deducir una fecha. Vuelve a preguntar por favor")
            }
